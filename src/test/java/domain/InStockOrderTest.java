...
public class InStockOrderTest {
    private InStockOrder order;
    ...
    @BeforeEach
    void setUp() {
        order = new InStockOrder(Id.newId(), Id.newId(), Id.newId(), buildCar());
    }
    @Test
    ...
    void markAsReady_fromPaid_toReadyForPickUp() {
        ...
        order.requestPayment();
        order.pay();
        assertTrue(order.markAsReady());
        assertEquals(ReadyForPickUpState.class, order.getStateType());
    }

    @Test
    void complete_fromReadyForPickUp_toCompleted() {
        order.approve();
        order.requestPayment();
        order.pay();
        order.markAsReady();
